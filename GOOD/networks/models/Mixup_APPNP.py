...
class Mixup_APPNP(GNNBasic):
    ...
    #     super(GAT, self).__init__()
        ...
        self.prop1 = APPNP_Conv(K, heads, app_alpha, self.att_alpha)
    ...
    def forward(self, *args, **kwargs) -> torch.Tensor:
        ...
        for i in range(self.K):
            x = self.prop1(h_a[-1], h_a[-1], edge_index)
        ...
        for i in range(self.K):
            if i ==0:
                new_h_a = self.prop1(h_a[0], h_mix[0], edge_index_a, edge_weight_a)
                new_h_b = self.prop1(h_b[0], h_mix[0], edge_index_b, edge_weight_b)
            else:
                new_h_a = self.prop1(h_a[-1], h_mix[-1], edge_index_a, edge_weight_a)
                new_h_b = self.prop1(h_b[-1], h_mix[-1], edge_index_b, edge_weight_b)
...
class APPNP_Conv(MessagePassing):
