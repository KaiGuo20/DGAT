...
from GOOD.utils.train import at_stage, SAM
import torch
...
class BaseOODAlg(ABC):
    ...
    def set_up(self, model: torch.nn.Module, config: Union[CommonArgs, Munch]):
        ...
        # self.optimizer = SAM(model.parameters(), base_optimizer, lr=config.train.lr, weight_decay=config.train.weight_decay)
        self.scheduler = torch.optim.lr_scheduler.MultiStepLR(self.optimizer, milestones=config.train.mile_stones,
                                                              gamma=0.1)

    # def backward(self, loss,ood_algorithm, raw_pred, targets, mask, node_norm, config,data, edge_weight, model):
    def backward(self, loss):
        ...
        # loss1 = ood_algorithm.loss_calculate(raw_pred, targets, mask, node_norm, config)
        # ood_algorithm.loss_postprocess(loss1, data, mask, config).backward()
