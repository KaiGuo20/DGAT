...
def process_configs(config: Union[CommonArgs, Munch]):
    ...
    model_dirname = f'{config.model.model_name}_{config.model.model_layer}l_{config.model.global_pool}pool_{config.model.dropout_rate}dp_{config.model.dim_hidden}dim_{config.model.attention_dropout_rate}_att{config.model.appnp_alpha}_alp{config.model.hops}_hop{config.model.pe_dim}_pe{config.model.num_heads}'
    train_dirname = f'{config.train.lr}lr_{config.train.weight_decay}wd_{config.train.max_epoch}'
    ...
    # config.ckpt_dir = opj(config.ckpt_dir, str(random_number))
