...
class GATConv(MessagePassing):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        print('att_dropout', self.dropout)
        ...
        if isinstance(in_channels, int):
            ...
            self.lin_src1 = Linear(heads * out_channels, heads * out_channels,
                                  bias=True, weight_initializer='glorot')
    ...
    def reset_parameters(self):
        ...
        # torch.nn.init.xavier_uniform_(self.att_src, gain=gain)
        # torch.nn.init.xavier_uniform_(self.att_dst, gain=gain)
    ...
    def forward(self, x: Union[Tensor, OptPairTensor], edge_index: Adj,
        ...
        # degs = degree(edge_index[0], num_nodes=num_nodes, dtype=torch.float)
        # degs = degs.clamp(min=1.0)
        #
        # # 计算规范化系数
        # norm = torch.pow(degs, -0.5)
        #
        # # 对节点特征进行规范化
        # x_src = x_src * norm.view(-1, 1)
