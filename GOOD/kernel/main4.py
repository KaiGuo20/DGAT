...
from GOOD.utils.initial import reset_random_seed, reset_random_seed_main
...
def initialize_model_dataset_main(config: Union[CommonArgs, Munch], seed) -> Tuple[torch.nn.Module, Union[dict, DataLoader]]:
    r"""
    Fix random seeds and initialize a GNN and a dataset. (For project use only)
...
def main():
    # yaml = YAML
    run_num = 1
    train_all_IID, valid_all_IID, test_all_IID = [], [], []
...
# ################################################3
    ...
    if config.task == 'train':
        ...
        print('valid_all_IID-mean', np.mean(valid_all_OOD))
...
def goodtg():
    try:
        train_IID, valid_IID, test_IID , OOD_test, train_OOD, valid_OOD, test_OOD = main()
        return train_IID, valid_IID, test_IID, OOD_test, train_OOD, valid_OOD, test_OOD
