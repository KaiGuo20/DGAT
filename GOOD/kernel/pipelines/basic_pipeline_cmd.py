...
from GOOD.utils.CMD import cmd,mmd, linear_CKA, CMD_NEW, CMD
...
class Pipeline:
    ...
    def train_batch(self, data: Batch, pbar) -> dict:
        ...
        model_output, last = self.model(data=data, edge_weight=edge_weight, ood_algorithm=self.ood_algorithm)
        ...
        # self.ood_algorithm.backward(loss, self.ood_algorithm, raw_pred, targets, mask, node_norm, self.config,data, edge_weight, self.model)
        self.ood_algorithm.backward(loss)

        return {'loss': loss.detach()}, last
    ...
    def train(self):
        ...
        for epoch in range(self.config.train.ctn_epoch, self.config.train.max_epoch):
            ...
            for index, data in pbar:
                ...
                # Parameter for DANN
                p = (index / len(self.loader['tra'
                                             'in']) + epoch) / self.config.train.max_epoch
                self.config.train.alpha = 2. / (1. + np.exp(-10 * p)) - 1

                # train a batch

                train_stat, last = self.train_batch(data, pbar)
            ...
            # checkpoints save
            counter = self.save_epoch(epoch, counter, epoch_train_stat, id_val_stat, id_test_stat, val_stat, test_stat, self.config)
            print('-----counter', counter)
            # --- scheduler step ---
            self.ood_algorithm.scheduler.step()
            # if self.config.train.type == 'Init':
            #     print('-------------Init')
            #     if epoch == self.config.train.max_epoch - 1:
            #         print(f"saving model_mlpinit at epcoh {epoch}")
            #         torch.save(self.model.state_dict(), f"./model_mlpinit.pt")

            if self.config.train.early_stop == 1:
                if counter == 100:
                    break
        # file_path = '/mnt/home/guokai1/workspace/GOOD/file/logits_mlp_raw.pt'
        # torch.save(logits, file_path)
        # mlp = torch.load('/mnt/home/guokai1/workspace/GOOD/file/logits_mlp_raw.pt')
        # gcn = torch.load('/mnt/home/guokai1/workspace/GOOD/file/logits_GCN_raw.pt')
        # cka = linear_CKA(mlp[data.test_mask].detach().cpu(), gcn[data.test_mask].detach().cpu())
        # print('cka----', cka)

        # train = F.normalize(last[data.train_mask], dim=-1)
        # test = F.normalize(last[data.test_mask], dim=-1)
        # train = torch.norm(last[data.test_mask])
        # test = torch.norm(last[data.test_mask])
        scaler = StandardScaler()
        train = scaler.fit_transform(last[data.test_mask].cpu().detach().numpy().reshape(-1, 1)).flatten()
        test = scaler.transform(last[data.test_mask].cpu().detach().numpy().reshape(-1, 1)).flatten()
        print('train', train.shape)
        # train = last[data.train_mask]
        # test = last[data.test_mask]
        # cmd_function = CMD_NEW()
        # cmd_value = cmd_function.mmatch(train, test, n_moments=3)

        # cmd_value = CMD(train, test, K=3)
    ...
    def evaluate(self, split: str):
        ...
        for data in pbar:
            ...
            print('data------\n', data)
            print('data.val_mask-----', data.test_mask)
            count_true = torch.sum(data.test_mask).item()
            ...
            data, targets, mask, node_norm = self.ood_algorithm.input_preprocess(data, targets, mask, node_norm,
                                                                                 self.model.training,
                                                                                 self.config)
            model_output, last = self.model(data=data, edge_weight=None, ood_algorithm=self.ood_algorithm)
    ...
    def load_task(self):
        ...
        if self.task == 'train':
            ...
            train_IID, valid_IID, test_IID , OOD_test, train_OOD, valid_OOD, test_OOD = self.config_model('test')
            return train_IID, valid_IID, test_IID, OOD_test, train_OOD, valid_OOD, test_OOD
        return
        ...
        if mode == 'test':
            ...
            if os.path.exists(self.config.id_test_ckpt):
                ...
                print(f'#IN#Checkpoint {id_ckpt["epoch"]}: \n-----------------------------------\n'
                      f' {self.config.metric.score_name}: {id_ckpt["train_score"]:.4f}\n'
                      f'TrTrainain Loss: {id_ckpt["train_loss"].item():.4f}\n'
                ...
                print(f'#IN#ChartInfo {id_ckpt["id_test_score"]:.4f} {id_ckpt["test_score"]:.4f} '
                      f'{ckpt["id_test_score"]:.4f} {ckpt["test_score"]:.4f} {ckpt["val_score"]:.4f}\n', end='')
            ...
            return id_ckpt["train_score"],id_ckpt["id_val_score"],id_ckpt["id_test_score"],id_ckpt["test_score"],\
                   ckpt["train_score"],ckpt["val_score"], ckpt["test_score"]


    def save_epoch(self, epoch: int, counter: int, train_stat: dir, id_val_stat: dir, id_test_stat: dir, val_stat: dir,
        ...
        ckpt = {
            ...
            'model': {
                ...
                # 'appnp_alpha': config.model.appnp_alpha
            },
        ...
        if prev_epoch:
            prev = prev_epoch[-1]
            prev_file = os.path.join(config.ckpt_dir, f'{prev}.ckpt')
