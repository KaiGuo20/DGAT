...
from GOOD.data.good_datasets.data_generate import get_bow,get_tf_idf_by_texts,get_word2vec
...
class GOODCiteseer(InMemoryDataset):
    r"""
    The GOOD-Pubmed dataset. Adapted from the `full Pubmed dataset
    ...
    def process(self):
        # dataset = CitationFull(root=self.root, name='Pubmed')
        # dataset = Planetoid(root=self.root,  name='Pubmed')
        # graph = dataset[0]
        dataset = torch.load('data/citeseer.pt')
        graph = dataset
    ...
    def load(dataset_root: str, domain: str, shift: str = 'no_shift', generate: bool = False):
        ...
        # dataset = GOODPubmed(root=dataset_root, domain=domain, shift=shift, generate=generate)
        if domain == "degree" and shift == "covariate":
            dataset = torch.load(
                f"/mnt/home/guokai1/workspace/GOOD/GOOD/data/good_datasets/{'citeseer'}_{'degree'}_{'covariate'}.pt",
                map_location='cpu')
        if domain == "word" and shift == "covariate":
            dataset = torch.load(
                f"/mnt/home/guokai1/workspace/GOOD/GOOD/data/good_datasets/{'citeseer'}_{'word'}_{'covariate'}.pt",
                map_location='cpu')
        if domain == "degree" and shift == "concept":
            dataset = torch.load(
                f"/mnt/home/guokai1/workspace/GOOD/GOOD/data/good_datasets/{'citeseer'}_{'degree'}_{'concept'}.pt",
                map_location='cpu')
        if domain == "word" and shift == "concept":
            dataset = torch.load(
                f"/mnt/home/guokai1/workspace/GOOD/GOOD/data/good_datasets/{'citeseer'}_{'word'}_{'concept'}.pt",
                map_location='cpu')
        ...
        data_full = dataset
        dataset = dataset[0]
        # dataset.x = get_bow(dataset.raw_texts)
        # dataset.x = get_word2vec(dataset.raw_texts)
        dataset.x, _ = get_tf_idf_by_texts(dataset.raw_texts, None, None, max_features=1433,
                                            use_tokenizer=False)
        ...
        meta_info.num_envs = (torch.unique(dataset.env_id) >= 0).sum()
        meta_info.num_train_nodes = dataset.train_mask.sum()

        # Define networks' output shape.
        dataset.task  = "Multi-label classification"
        dataset.metric = "Accuracy"
        dataset.processed_dir = "/mnt/ufs18/home-246/guokai1/workspace/GOOD/storage/datasets/GOODCiteseer/degree/processed"
        if dataset.task == 'Binary classification':
            meta_info.num_classes = dataset.y.shape[1]
        elif dataset.task == 'Regression':
            meta_info.num_classes = 1
        elif dataset.task == 'Multi-label classification':
            meta_info.num_classes = torch.unique(dataset.y).shape[0]
...
if __name__ == '__main__':
    dataset = GOODPubmed(root='data', domain='word', shift='covariate', generate=True)